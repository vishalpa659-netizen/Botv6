...
def generate_signal(symbol, df, regime):
    ...
    # Loss streak guard
    conservative = learn_streak >= 3
    stance = (
        "be CONSERVATIVE — only BUY or SELL if ALL signals strongly agree"
        if conservative else
        "be DECISIVE — favour BUY or SELL over HOLD when probability >= 70%"
    )

    prompt = f"""You are an expert forex and crypto trading signal AI.

Market Data:
- Symbol: {sym_display}
- Price: {entry}
- Trend: {trend}
- MTF Confirmation: {mtf_confirmation}
...
- Past Wins: {learn_wins} | Past Losses: {learn_losses} | Loss Streak: {learn_streak}

Decision rules:
1. {stance}
2. BUY when: Trend=BULLISH + MTF=BULLISH + MACD above Signal + RSI below 75
3. SELL when: Trend=BEARISH + MTF=BEARISH + MACD below Signal + RSI above 25
4. HOLD only when signals clearly conflict or regime is VOLATILE
5. Probability >= 80% means strongly prefer BUY or SELL

Reply with exactly one word only — BUY, SELL, or HOLD:"""

    try:
        ai_model = client.model("meta-llama/Llama-3.3-70B-Instruct")
        result = ai_model.run(prompt)
        raw = (result.output or "").strip().upper()
        signal_direction = "HOLD"
        for word in raw.split():
            clean = word.strip(".,!?:")
            if clean in ("BUY", "SELL", "HOLD"):
                signal_direction = clean
                break
        print(f"[Bytez AI] {sym_display} → {signal_direction} (raw: {raw[:50]})")
    except Exception as e:
        print(f"[Bytez AI Error] {e} — using indicator fallback")
        if trend == "BULLISH" and mtf_confirmation == "BULLISH":
            signal_direction = "BUY"
        elif trend == "BEARISH" and mtf_confirmation == "BEARISH":
